...
http.createServer(function(req,res) {
    ...
    if(pathName == "/data") {
        ...
        var pt1 = path.join(__dirname,'../login.html');
        var pt2 = path.join(__dirname,'../list.html');
        ...
        var pt = path.join(__dirname,'../chapterList.html');
        showPage(res,pt);
    } else if(pathName == '/detail') {
        var pt = path.join(__dirname,'../chapter.html');
        showPage(res,pt);        
    } else if(pathName == '/login') {
        var pt = path.join(__dirname,'../login.html');
        showPage(res,pt);
        ...
        req.on('end',function() {
            ...
            if(dataObj.username == userList[0].username && dataObj.pwd == userList[0].pwd) {
                var pt = path.join(__dirname,"../list.html");
                res.setHeader("Set-Cookie","username=admin;max-age=3600");
                showPage(res,pt);
            } else {
                res.end("Your message is wrong,please input again!");
            }          
        })
        ...
        fs.readFile(path1+ req.url,function(err,data) {
            if(!err) {
                res.end(data);
            } else {
                console.log(err);
